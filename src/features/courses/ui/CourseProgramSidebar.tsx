...
function flattenModules(modules: ProgramModuleItem[]): ProgramModuleItem[] {
  const result: ProgramModuleItem[] = [];
...
function collectUnits(modules: ProgramModuleItem[]): ProgramUnitItem[] {
  const units: ProgramUnitItem[] = [];
...
function calcProgressFallback(
  ...
  const allUnits = collectUnits(modules);
...
export function CourseProgramSidebar({
  ...
  const allUnitIds = useMemo(
    () => collectUnits(modules).map((unit) => unit.id),
    [modules],
  );
    ...
    <aside className={clsx("skillhub-panel rounded-[1.75rem] p-4", className)}>
      ...
      <div className="space-y-3">
        {(() => {
          ...
          const countSubtreeUnits = (node: ProgramModuleItem): number => {
            const own = node.units.length;
            const nested = (node.children ?? []).reduce(
              (sum, child) => sum + countSubtreeUnits(child),
              0,
            );
            return own + nested;
          };

          const countSubtreeCompleted = (node: ProgramModuleItem): number => {
            const own = node.units.reduce((count, unit) => {
              const done = courseId
                ? isCompleted(unit.id)
            ...
            const nested = (node.children ?? []).reduce(
              (sum, child) => sum + countSubtreeCompleted(child),
              0,
            );
            return own + nested;
          };
          ...
          const renderModule = (
            ...
            const totalUnits = countSubtreeUnits(module);
            const completedUnits = countSubtreeCompleted(module);
          ...
          return modules.map((module) => renderModule(module, 0));
        })()}
