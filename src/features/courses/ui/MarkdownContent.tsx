...
function createComponents(showImageCaption: boolean): Components {
  return {
    h1: ({ children }) => (
      <h1 className="mb-3 mt-6 text-xl font-bold text-black first:mt-0">
        {children}
      </h1>
    ),
    h2: ({ children }) => (
      <h2 className="mb-2 mt-5 text-lg font-semibold text-black first:mt-0">
        {children}
      </h2>
    ),
    h3: ({ children }) => (
      <h3 className="mb-2 mt-4 text-base font-semibold text-black first:mt-0">
        {children}
      </h3>
    ),
    p: ({ children }) => (
      <p className="mb-3 text-sm leading-6 text-[var(--muted)] last:mb-0">
        {children}
      </p>
    ),
    strong: ({ children }) => (
    ...
    ul: ({ children }) => (
      <ul className="mb-3 space-y-1 pl-5 text-sm text-[var(--muted)] [list-style:disc]">
        {children}
      </ul>
    ),
    ol: ({ children }) => (
      <ol className="mb-3 space-y-1 pl-5 text-sm text-[var(--muted)] [list-style:decimal]">
        {children}
      </ol>
    ),
...
export function MarkdownContent({
  ...
  return (
    <div className="mt-3">
      <ReactMarkdown components={createComponents(showImageCaption)}>
        {content}
      </ReactMarkdown>
    </div>
  );
